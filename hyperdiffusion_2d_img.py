...
class HyperDiffusion_2d_img(torch.nn.Module):
        ...
        for inverse_step_id in range(n_inverse_steps):
            ...
            if update_grad:
                ...
                for code_idx, single_mlp in enumerate(mlp.models):
                    ...
                    current_idx = 0
                    print(len(grad_inner), len(list(single_mlp.parameters())), len(prior_grad[code_idx]))
    ...
    def training_step(self, train_batch, optimizer, code_optimizer_state, global_step, save_to_disk):
        ...
        input_img = train_batch['gt_img'][16].clone().detach().view(64, 64, 3).permute(2, 0, 1).cuda()
        ...
        loss_mse = 5 * loss_mse / self.norm_factor
