...
class HyperDiffusion_2d_img(torch.nn.Module):
    ...
    def training_step(self, train_batch, optimizer, code_optimizer_state, MLP, global_step, save_to_disk):
        ...
        #start = time.time()
        print('!!!!! ', train_batch['coords'][0].unsqueeze(0).shape)
        inv_loss, code_reg, psnr, code_optim_state_ = self.inverse_code_1b1(train_batch['gt_img'], train_batch['coords'][0].unsqueeze(0),
                                                                            code_list_,
    ...
    def test_step(self):
        ...
        wandb.log({"generated_images_grid": wandb.Image(grid, caption="16x16 Image Grid")})
