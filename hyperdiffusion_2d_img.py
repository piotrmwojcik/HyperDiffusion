...
class HyperDiffusion_2d_img(torch.nn.Module):
    ...
    def inverse_code_1b1(self, gt_imgs, coords, code_, optimizer_state, prior_grad, cfg, mlp):
        ...
        for inverse_step_id in range(n_inverse_steps):
            ...
            if update_grad:
                ...
                prior_grad_ = Config.get('inverse_amplify') * torch.cat(prior_grad, dim=0).cuda()
                ...
                for grad, param in zip(grad_inner, mlp.parameters()):
                    ...
                    grad = grad.view(-1)
                    #print(grad.shape, grad.norm(p=2).item(), prior_grad_[current_idx:current_idx + num_params].norm(p=2).item())
    ...
    def training_step(self, train_batch, coords, optimizer, code_optimizer_state, global_step, save_to_disk, MLP, epoch):
        ...
        loss_mse = Config.get("loss_weight") * loss_mse / (self.norm_factor / 333.37)
        self.logger.log({"global_step": global_step, "norm factor": (1.0 / (self.norm_factor.item()) / 333.37)})
    ...
    def validation_step(self, epoch):
        ...
        input = get_grid(32, 32, b=0).unsqueeze(0)
        result = siren({'coords': input})
        print(result['model_out'].shape)
        # print(img)
        img = dataio.lin2img(result['model_out'], (32, 32))
        img = dataio.rescale_img((img + 1) / 2, mode='clamp')
        img = (img * 255).byte()
        #print(img)
        #print('!!!')
