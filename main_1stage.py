...
def main(cfg: DictConfig):
    ...
    if Config.get('model_resume_path') is not None:
        ...
        if 'model' in checkpoint['diffuser']:
            new_model_dict = {}
            for key, value in checkpoint['diffuser']['model'].items():
        ...
        for epoch in range(epoch_start, num_epochs):
            ...
            with tqdm(train_dl, desc=f"Epoch {epoch + 1}/{num_epochs}", unit="batch") as pbar:
                for batch_idx, data in enumerate(pbar):
                    ...
                    loss, code_optimizer_state_ = diffuser.training_step(data, optimizer, code_optimizer_state, global_step, save_to_disk, MLP)  # Forward pass
                    outputs.append(loss)
                    global_step += 1
