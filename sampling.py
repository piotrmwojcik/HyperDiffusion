...
def get_points_from_vox(path):
    binvox = load_binvox(path)
    voxel_model_128 = binvox.data
    voxel_model_128 = voxel_model_128.astype(int)
        ...
        for j in range(dim_voxel):
            for k in range(dim_voxel):
                voxel_model_temp[i, j, k] = np.max(
                    voxel_model_128[i * multiplier:(i + 1) * multiplier, j * multiplier:(j + 1) * multiplier,
    ...
    temp_range = list(range(1, dim_voxel - 1, 4)) + list(range(2, dim_voxel - 1, 4)) + list(
        range(3, dim_voxel - 1, 4)) + list(range(4, dim_voxel - 1, 4))
    for j in temp_range:
        ...
        for i in temp_range:
            ...
            for k in temp_range:
                if (batch_size_counter >= batch_size): break
                if (np.max(voxel_model_temp[i - 1:i + 2, j - 1:j + 2, k - 1:k + 2]) != np.min(
                        voxel_model_temp[i - 1:i + 2, j - 1:j + 2, k - 1:k + 2])):
                    si, sj, sk = sample_point_in_cube(
                        voxel_model_128[i * multiplier:(i + 1) * multiplier, j * multiplier:(j + 1) * multiplier,
                        k * multiplier:(k + 1) * multiplier], voxel_model_temp[i, j, k], halfie)
    ...
    dims = binvox.dims
    translate = binvox.translate
    scale = binvox.scale

    x = sample_points_64[:, 0]
    y = sample_points_64[:, 1]
    z = sample_points_64[:, 2]
    ...
    sample_points_64 = np.stack((x, y, z), axis=-1).reshape(-1, 3).astype(np.float32)

    return sample_points_64, sample_values_64
...
def main():
    ...
    while True:
        item_flag = True
        try:
            idx, exceed_64_flag, exceed_32_flag, sample_points_64, sample_values_64, sample_points_32, sample_values_32, sample_points_16, sample_values_16, sample_voxels = q.get(
                True, 1.0)
        except queue.Empty:
            item_flag = False
...
def plot_occupied_voxel(grid_size, points, occs):
    grid = np.zeros((grid_size,grid_size,grid_size))
    coords = (points + 0.5) * grid_size
    # occs = 1 - occs
    indices = np.clip(coords.astype(int), 0, grid_size - 1)

    grid[indices[:, 0], indices[:, 1], indices[:, 2]] = occs.flatten()
    threshold = 0.5
    grid[:,:30,:] = 0
    voxel_data = grid > threshold  # Boolean array for voxels above threshold
    fig = plt.figure(figsize=(10, 10))
    ax = fig.add_subplot(111, projection='3d')
    ax.voxels(voxel_data, facecolors='cyan')
...
def main2():
    path = '/Users/kacpermarzol/PycharmProjects/hyperdiffusionproject/HyperDiffusion/data/02958343/1a0bc9ab92c915167ae33d942430658c/models/model_normalized.solid.binvox'
    points, occs = get_points_from_vox(path)

    print(points)
    print(occs.shape)

    plot_occupied_voxel(128,points,occs)
